...
function scopedIds(
  user: { roles: Array<{ role: string; federationId: string | null; competitionId: string | null }> } | null,
  ...
  register: async (app) => {
    ...
    app.post('/competitions', { preHandler: requireAuth() }, async (req, reply) => {
      ...
      if (!assertSupportedTimezone(data.timezone)) {
        return reply.code(400).send({
          error: { code: 'invalid_timezone', message: 'Unsupported IANA timezone', requestId: req.requestId },
        });
      }
      ...
      try {
        ...
        log.info({ competitionId: competition.id, federationId: data.federationId }, 'competition created');
        return reply.code(201).send({ competition });
      } catch (err) {
        if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === 'P2002') {
          return reply.code(409).send({
            error: { code: 'code_taken', message: 'Competition code already exists in this federation', requestId: req.requestId },
          });
        }
        throw err;
      }
