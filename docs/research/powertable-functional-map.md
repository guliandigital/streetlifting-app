...
## Federation home page (PowerTable "Начальная страница")
...
- **Telegram bot prompt** — "@PowerTable_bot, отправьте код NNNNNNNNN для уведомлений"
...
## Settings / feedback page (PowerTable "Обращения, настройки")
...
## Tab strip (PowerTable opened tabs)

PowerTable runs many tabs simultaneously: Начальная, Соревнования, Спортсмены, Склад, Номинации спортсменов, athlete card. Modern web doesn't need this — TanStack Router handles deep linking and back/forward. We **do not** replicate the tabbed window.

## Connection-quality widget

Bottom-left of PowerTable: "Среднее значение качества связи с сервером [302мс]" with rolling history.
...
## What we add (no PowerTable equivalent)
...
- Dark mode (PowerTable is light-only)
